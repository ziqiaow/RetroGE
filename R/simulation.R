...
dat=simFit(ncontrol=500, ncase=500,
           beta0=-4.8, betaG_normPRS=0.450,betaE_bin=0.15, betaE_norm=-0.07,betaGE_normPRS_bin=0, betaGE_normPRS_norm=0,E_bin_freq=0.745,strata=F)
...
for(i in 1:sim_N){
  ...
  res[[i]]=prs_e_function_gr(data=dat0,formula=D ~ prs+envir1 +envir2+envir1:prs+envir2:prs,
                             formula_prs = prs~1,
                             facVar=NULL,
                             numDeriv=F,
                             side0=2)
  ...
  #nonparametric
  spmleFull = spmle(D=dat$D, G=dat$G, E=dat$E,pi1=0)
...
#look at the population disease prevalence
dat=simFit(ncontrol=500, ncase=500,
           beta0=-7, betaG_normPRS=0.450,betaE_bin=0.15, betaE_norm=-0.07,betaGE_normPRS_bin=0, betaGE_normPRS_norm=-0.4,E_bin_freq=0.745,strata=T,interact_strata_prs = F,beta_strata=c(0.1,0.3,0.5),mu_strata=c(1,0.2,0.3,0.2))
...
for(i in 1:sim_N){
  ...
  res[[i]]=prs_e_function_gr(data=dat0,formula=D ~ prs+envir1 +envir2+factor(s1)+s2+envir1:prs+envir2:prs,
  ...
  spmleFull = spmle(D=dat$D, G=dat$G, E=cbind(dat$E,s[,c(3,4,2)]),pi1=0)
  res_spmle[[i]]=summary(spmleFull)$coefficients
...
#look at the population disease prevalence
dat=simFit(ncontrol=500, ncase=500,
           beta0=-6, betaG_normPRS=0.45,betaE_bin=0.15, betaE_norm=-0.07,betaGE_normPRS_bin=0, betaGE_normPRS_norm=-0.4,E_bin_freq=0.745,strata=T,interact_strata_prs = T,beta_strata=c(0.1,0.3,0.5),beta_strata_interact = c(0,0.1,0.2),mu_strata=c(1,0.2,0.3,0.2))
...
for(i in 1:sim_N){
  ...
  res[[i]]=prs_e_function_gr(data=dat0,formula=D ~ prs+envir1 +envir2+factor(s1)+s2+envir1:prs+envir2:prs+factor(s1):prs+s2:prs,
                             formula_prs=prs~factor(s1)+s2,
                             facVar=c("s1"),
                             initial_empirical=F,
                             numDeriv=F,
...
simFit <- function(ncontrol=500, ncase=500,
                   beta0=-5, betaG_normPRS=0.450,betaE_bin=0.143, betaE_norm=-0.019,betaGE_normPRS_bin=0, betaGE_normPRS_norm=-0.40,E_bin_freq=0.745,strata=TRUE,interact_strata_prs=FALSE,beta_strata=c(0.1,0.1,0.1),beta_strata_interact=c(0.4,0.4,0.4),sigma_strata=c(0.25,0.5,1),mu_strata=c(1,0.2,0.3,0.2)){
  
  n_pop=1000000
  library(MASS)
  if(strata==TRUE){
    my_mu1 <- c(0, 0)                                   # Specify the means of the variables
    my_Sigma1 <- matrix(c(1, 0.4, 0.4, 1),                 # Specify the covariance matrix of the variables
                        ncol = 2)
  ...
  prs <- as.numeric(as.vector(scale(prs, center = TRUE, scale = F)))
  
  if(strata==TRUE){
    if(interact_strata_prs==TRUE){
      xb <- beta0 + betaG_normPRS*prs + betaE_bin*E_sim_bin + betaE_norm*E_sim_norm + betaGE_normPRS_bin*E_sim_bin*prs+betaGE_normPRS_norm*E_sim_norm*prs+S_sim.tmp[,-1] %*% beta_strata + beta_strata_interact[1]*prs*S_sim.tmp[,2] +beta_strata_interact[2]*prs*S_sim.tmp[,3] +beta_strata_interact[3]*prs*S_sim.tmp[,4]
    } else {
      xb <- beta0 + betaG_normPRS*prs + betaE_bin*E_sim_bin + betaE_norm*E_sim_norm + betaGE_normPRS_bin*E_sim_bin*prs+betaGE_normPRS_norm*E_sim_norm*prs
    }
  }else{
    xb <- beta0 + betaG_normPRS*prs + betaE_bin*E_sim_bin + betaE_norm*E_sim_norm + betaGE_normPRS_bin*E_sim_bin*prs+betaGE_normPRS_norm*E_sim_norm*prs
  }
  ...
  D_sim <- rbinom(n = n_pop, size = 1, prob = prob)  # 1=case, 0=control
  ...
  if(strata==TRUE){
    ...
    S = rbind(as.matrix(S_sim[id1,]),as.matrix(S_sim[id2,]))}else{S=NULL}
  
  #### Bind D, G, and E into a list
  dat = list(D=D, G=G, E=E, S=S,prevelance=mean(D_sim),mean_prs=mean_prs,sd_prs=sd_prs)
  return(dat)
...
#look at the population disease prevalence
dat=simFit(ncontrol=500, ncase=500,
           beta0=-5, betaG_normPRS=0.45,betaE_bin=0.143, betaE_norm=-0.019,betaGE_normPRS_bin=0, betaGE_normPRS_norm=-0.4,E_bin_freq=0.745,strata=T,interact_strata_prs = F,beta_strata=c(0.1,0.3,0.5),beta_strata_interact = c(0,0.1,0.2))
...
for(i in 1:sim_N){
  set.seed(i)
  dat=simFit(ncontrol=500, ncase=500,
             beta0=-5, betaG_normPRS=0.45,betaE_bin=0.15, betaE_norm=-0.07,betaGE_normPRS_bin=0, betaGE_normPRS_norm=-0.4,E_bin_freq=0.745,strata=T,interact_strata_prs = F,beta_strata=c(0.1,0.3,0.5),beta_strata_interact = c(0,0.1,0.2))
  dat0=cbind(dat$D,dat$G,dat$E,dat$S)
  colnames(dat0)=c("D","prs","envir1","envir2","s1","s2")
  dat0=data.frame(dat0)
  dat0$s1=factor(dat0$s1)
  ...
  #nonparametric
  spmleFull = spmle(D=dat$D, G=dat$G, E=dat$E,pi1=dat$prevelance)
  res_spmle[[i]]=summary(spmleFull)$coefficients
...
simFit <- function(ncontrol=500, ncase=500,
  ...
  if(strata==TRUE){
    if(interact_strata_prs==TRUE){
      ...
      xb <- beta0 + betaG_normPRS*prs + betaE_bin*E_sim_bin + betaE_norm*E_sim_norm + betaGE_normPRS_bin*E_sim_bin*prs+betaGE_normPRS_norm*E_sim_norm*prs+S_sim.tmp[,-1] %*% beta_strata
    }
  }else{
    xb <- beta0 + betaG_normPRS*prs + betaE_bin*E_sim_bin + betaE_norm*E_sim_norm + betaGE_normPRS_bin*E_sim_bin*prs+betaGE_normPRS_norm*E_sim_norm*prs
  }
...
#look at the population disease prevalence
dat=simFit(ncontrol=500, ncase=500,
           beta0=-5, betaG_normPRS=0.45,betaE_bin=0.143, betaE_norm=-0.019,betaGE_normPRS_bin=0, betaGE_normPRS_norm=-0.4,E_bin_freq=0.745,strata=T,interact_strata_prs = F,beta_strata=c(0,0.3,0.5),beta_strata_interact = c(0,0.1,0.2))
#Disease prevalence:  0.00999
...
#look at the population disease prevalence
dat=simFit(ncontrol=500, ncase=500,
           beta0=-6, betaG_normPRS=0.45,betaE_bin=0, betaE_norm=0,betaGE_normPRS_bin=0, betaGE_normPRS_norm=0,E_bin_freq=0.745,strata=T,interact_strata_prs = T,beta_strata=c(0.1,0.3,0.5),beta_strata_interact = c(0,0.1,0.2),mu_strata=c(1,0.2,0.3,0.2))
#Disease prevalence:  0.011257
...
function_caseonly1 <- function(data_sim,strata=FALSE){
  ...
  fit_caseonly <- lm(prs[D==1] ~ factor(data_sim$S[D==1,1])+data_sim$S[D==1,2])
...
for(i in 1:sim_N){
  ...
  res[[i]]=prs_e_function_gr(data=dat0,formula=D ~ prs+factor(s1)+s2+factor(s1):prs+s2:prs,
                             formula_prs=prs~factor(s1)+s2,
                             facVar=c("s1"),
                             initial_empirical=F,
                             numDeriv=F,
  ...
  spmleFull = spmle(D=dat$D, G=dat$G, E=s[,c(3,4,2)],pi1=0)
  res_spmle[[i]]=summary(spmleFull)$coefficients
