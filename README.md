...
Note that for real data applications, it is suggested to **normalize the continuous variables** that have largely varied scales for a stable numerical derivation. Simply use this R code to normalize the continuous variable x: scale(x,center = TRUE, scale = TRUE).
